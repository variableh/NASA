...
> **Status:** Completed — baseline tabular ML study
...
- **Health-state classification:** Using project-defined health states, Logistic Regression detected **76% of engines in the Critical state** with **0.86 precision** on the test engines — and never dismissed a Critical engine as Healthy.
...
- **Main limitation:** the study uses simulated FD001 data with a single operating condition and a single degradation scenario.
...
| **Evaluation** | MAE, RMSE, R², and the asymmetric PHM 2008 challenge score for regression; Balanced Accuracy, Macro F1, Critical precision/recall for classification |
...
**C-MAPSS** (Commercial Modular Aero-Propulsion System Simulation) is NASA's thermodynamic simulation tool for a large commercial turbofan engine. FD001 uses a single **HPC degradation scenario based on flow and efficiency degradation**; the fault is otherwise unspecified. Sensor responses are generated by the C-MAPSS engine model, and each trajectory ends when a health index — the minimum of several operational margins — reaches zero.
...
The project uses **FD001**, the simplest of the four C-MAPSS subsets: it uses a **single operating condition** and the single HPC degradation scenario described above. The broader C-MAPSS challenge dataset includes subsets with multiple operating conditions.
...
|---|---|---|
| **[01 — Exploratory Analysis & Preprocessing](01_analysis.ipynb)** | Turn raw FD001 files into model-ready, leakage-free datasets | Full sensor audit (identifies 7 constant/near-constant channels) · RUL target construction with the 125-cycle cap · engine-level 80/20 split before any preprocessing · scaler fitted on training engines only · exports standardized processed datasets consumed by both modeling notebooks |
...
- **Metrics matched to the operational objective.** Regression reports MAE/RMSE/R² alongside the asymmetric scoring function introduced for the PHM 2008 challenge: it penalizes over-predicting remaining life more heavily than under-predicting. It is relevant to C-MAPSS, not a universal predictive-maintenance metric. Classification prioritizes Critical Recall, with Balanced Accuracy and Macro F1 guarding against imbalance-blind wins.
...
Validation results, sorted by NASA score. The asymmetric scoring function introduced for the PHM 2008 challenge penalizes *late* predictions (over-estimating remaining life) more heavily than early ones, reflecting the operational preference for early prediction — **lower NASA scores are better** (↓ = lower is better, ↑ = higher is better):
...
**Selected model: Random Forest (tuned).** It was selected for the strongest overall validation profile under this project's evaluation criteria: best RMSE and R², second-best NASA score, and MAE within 0.19 cycles of the leader. Its tuning also improved every validation metric (NASA 7.96 → 7.34, RMSE 17.21 → 16.85, R² 0.83 → 0.84, MAE 12.56 → 12.37). XGBoost kept the best MAE but a clearly worse NASA score; all results are kept as measured, including where NASA-score-only tuning made models *less* accurate overall.
...
- **The most important features are physically plausible for the simulated degradation scenario.** `sensor_11` (static pressure at the HPC outlet, Ps30) dominates the tree models' importances, followed by `sensor_4` (LPT outlet temperature, T50), `sensor_9`, and `sensor_12` — gas-path channels consistent with sensor responses expected under the simulated HPC degradation scenario.
...
- **The classification task adds an operational lens regression doesn't provide.** Continuous RUL is harder to act on than a severity band; using the project's thresholds, the classifier detects 76% of engines in the Critical state with 0.86 precision and never dismisses a Critical engine as Healthy. Conversely, classification loses granularity ("31 cycles" and "100 cycles" are both Warning), which is exactly what regression preserves.
...
- **Reproducibility:** fixed seeds (42) for every stochastic component, a deterministic notebook order, and standardized processed datasets shared by the modeling notebooks.
- **Known limitations, kept visible:** simulated FD001 data with one operating condition and one HPC degradation scenario · 100-engine test set, so metrics carry real statistical uncertainty · training labels capped at 125 cycles, which the model cannot predict beyond (11 test engines sit above the cap) · project-defined class thresholds are design assumptions, not dataset-derived values.
...
AI-assisted tools were utilized to support code refactoring and documentation. The final implementation, methodology, experimental design, and results have been thoroughly reviewed and validated against the project requirements and source data.
